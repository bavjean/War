...
class Game {
	...
	play() {
		while (playerOne.length > 0 && playerTwo.length > 0 && this.round < 10000) {
			...
			if (card1.score > card2.score) {
				...
				this.goToWar()
					...
					playerTwo = []
					this.cardsInPlay = []
					console.log("Player two does not have enough cards to go to war.")
				} else if (playerTwo.length > playerOne.length) {
					playerTwo.push.apply(playerTwo, playerOne)
					playerTwo.push.apply(playerTwo, this.cardsInPlay)
					playerOne = []
					this.cardsInPlay = []
					console.log("Player one does not have enough cards to go to war.")
				}
			...
			if (this.round >= 10000) {
			console.log("Game taking too long... reshuffling cards")
			this.round = 1

			this.shuffle(playerOne)
			this.shuffle(playerTwo)
			}
		}

		if (playerOne.length === 0) console.log("Player two wins the game!")
		if (playerTwo.length === 0) console.log("Player one wins the game!")
